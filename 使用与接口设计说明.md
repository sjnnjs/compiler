...
当前阶段是直接打印 token。后续接入语法分析时，应将输出行为改造成结构化 token：
...
后续使用 Bison 时，`yylex()` 应返回 token 类型，并通过 `yylval` 传递语义值，通过 `yylloc` 或自定义字段传递行号。此时 `print_token()`、`print_text_token()` 等函数可以保留为调试输出函数，但正式编译流程不应依赖它们。

### 6.2 语法分析接口

语法分析阶段输入 token 流，输出抽象语法树 AST。
