...
int main(int argc, char **argv)
{
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <source-file> <output-ir-file>\n", argv[0]);
        return 1;
    }
    ...
    if (lexical_error_count == 0 && syntax_error_count == 0 && parse_result == 0) {
        semantic_errors = semantic_analyze(syntax_root);
        if (semantic_errors == 0) {
            ir_result = ir_generate(syntax_root, argv[2]);
        }
    }
    /* The process exits immediately after analysis; keeping the tree avoids
       double-free risks from shared sibling links in generated AST fragments. */

    return (lexical_error_count || syntax_error_count || parse_result || semantic_errors || ir_result) ? 2 : 0;
}
